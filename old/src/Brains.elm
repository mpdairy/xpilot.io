...
nearestObj (GameObject self) objects =
    case
        (List.foldr
            (\(GameObject obj) closest ->
                if obj.id == self.id then
                    closest
                else
                    let
                        newd =
                            distanceComp self.pos obj.pos
                    in
                        case closest of
                            Nothing ->
...
bulletDanger spos svel bpos bvel =
    let
        tx =
            -1 * (bpos.x - spos.x) / (bvel.x - svel.x)

        ty =
            -1 * (bpos.y - spos.y) / (bvel.y - svel.y)
    in
        if tx < 0 || ty < 0 then
            0
        else
            1000 * (1 / (max 0.001 (abs (tx - ty))))


mostDangerousBullet :
    GameObject a XpilotObject c
    -> List (GameObject a XpilotObject c)
...
mostDangerousBullet (GameObject self) bullets =
    List.foldr
        (\(GameObject b) mdb ->
            let
                nd =
                    bulletDanger self.pos self.vel b.pos b.vel
            in
                case mdb of
                    Nothing ->
                        Just ( nd, GameObject b )
    ...
    case ship.self of
        Ship self ->
            let
                ...
                cShip =
                    nearestObj (GameObject ship) ships
            in
                case cShip of
                    Nothing ->
                        ShipControls
...
sidEvadeBullet (GameObject ship) danger (GameObject bullet) =
    let
        rm =
            rotationMatrix (aimDir (Vector 0 0) ship.vel)

        bpos =
            rotatePointWithMatrix rm (vectorSub bullet.pos ship.pos)

        bvel =
            rotatePointWithMatrix rm (vectorSub bullet.vel ship.vel)
                        ...
                        case aimAtShip meta.shotVel (GameObject ship) (GameObject s) of
                            ...
                            Just aimdir ->
                                ...
                                then
                                    ...
                                    ShipControls
                                        { thrust =
                                            if abs (ship.vel.y + ship.vel.x) < 0.1 then
                                                1.0
                                            else
                                                0
                                        , shoot = False
