    ...
    case msg of
        ...
        MaybeThrustEmber diff ownerID pos vel strength rand ->
            ...
            in
                ...
                , case rand of
                    [ rand ] ->
                        if rand < diff * meta.shipEmberRate then
...
bulletControl :
    ...
    -> ( GameObject XpilotMeta XpilotObject XpilotMsg, List (GameMessage XpilotMeta XpilotObject XpilotMsg) )
bulletControl diff (World { meta }) (GameObject object) =
    case object.self of
        Bullet bullet ->
            let
                newShotLife =
                    bullet.shotLife + diff
            in
                if newShotLife > meta.shotLife then
                    ...
                    let
                        newBullet =
                            Bullet { bullet | shotLife = newShotLife }
                    in
...
shipCollision (World world) (GameObject ship) (GameObject other) =
    case other.self of
        ...
        Wall _ ->
            let
                xLap =
                    spanOverlap ship.boundingBox.xSpan other.boundingBox.xSpan

                yLap =
                    spanOverlap ship.boundingBox.ySpan other.boundingBox.ySpan
                ...
                lethal =
                    ((ship.vel.x ^ 2 + ship.vel.y ^ 2) ^ 0.5) > 0.8
            in
                ( GameObject
                    { ship
                        | vel = newVel
        ...
        Ship self ->
            let
                ...
                (ShipControls control) =
                    self.brain (World world) (GameObject ship)

                newdir =
                    within2pi
                        (self.dir
                            + (clamp ((-1) * world.meta.shipMaxTurn)
            ...
            in
                ...
                , List.concat
                    [ (if shooting then
                        let
                            rgunPos =
                                rotatePointWithMatrix rMatrix self.gunPos
                            ...
                            shotPos =
                                (vectorAdd (vectorAdd ship.pos rgunPos)
                                    shotVel
                                )
                        in
                    ...
                    , (if control.thrust > 0 then
                        let
                            rthrustPos =
                                rotatePointWithMatrix rMatrix self.thrusterPos

                            thrustPos =
                                vectorAdd ship.pos rthrustPos

                            thrustVel =
                                vectorAdd (vectorMultScalar 0.5 ship.vel)
...
newShip meta id name dir pos brain =
    let
        shipshape =
            [ (Vector -7 6), (Vector 7 6), (Vector 0 -15) ]
...
keyboardBrain (World { meta }) _ =
    ...
    in
        ShipControls
            { thrust =
                (if List.member Thrust cs then
                    meta.shipMaxThrust
                 else
                    0
                )
            , turn =
                (if List.member TurnRight cs then
                    (-1 * meta.shipMaxTurn)
                 else
                    0
                )
                    + (if List.member TurnLeft cs then
                        meta.shipMaxTurn
                       else
                        0
                      )
...
handleKeyDown =
    handleKey addControlAction


handleKeyUp : KeyCode -> World XpilotMeta a b -> World XpilotMeta a b
handleKeyUp =
    handleKey rmControlAction


handleKey :
    (ControlAction -> XpilotMeta -> XpilotMeta)
    -> KeyCode
...
handleKey f k (World world) =
    let
        maybeAction =
            (Dict.get k world.meta.keyboardControls)
    in
        World
            (case maybeAction of
                Nothing ->
...
emberControl diff (World { meta }) (GameObject object) =
    case object.self of
        Ember ember ->
            let
                newStrength =
                    ember.strength - meta.emberReduceRate * diff
            in
