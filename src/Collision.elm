...
type alias Span = { low : Float
                  , high : Float }

type alias BoundingBox = { xSpan : Span
                         , ySpan : Span }
...
spanOverlap s1 s2 = max 0.0 ((min s1.high s2.high) - (max s1.low s2.low))
...
xSpan pos shape =
    case shape of
        ...
        Polygon points ->
            case points of
                [] -> { low = pos.x, high = pos.x }
                (firstP::_) -> List.foldr (\p span -> { low = min (p.x + pos.x) span.low
...
boundingBoxCollision box1 box2 = (spanCollision box1.xSpan box2.xSpan)
            ...
            case points of
                ...
                (p::_) ->
                    let bounds =
                            List.foldr
                            (\ point bounding ->
                                 { xSpan = { low = min bounding.xSpan.low point.x
                                           , high = max bounding.xSpan.high point.x }
                                 , ySpan = { low = min bounding.ySpan.low point.y
                            ...
                            points
                    in
                        { xSpan = { low = pos.x + bounds.xSpan.low
                                  , high = pos.x + bounds.xSpan.high }
                        , ySpan = { low = pos.y + bounds.ySpan.low
                                  , high = pos.y + bounds.ySpan.high } }


segmentAngle : Vector -> Vector -> Float
segmentAngle p1 p2 = atan2 (p2.y - p1.y) (p2.x - p1.x)
