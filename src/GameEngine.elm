...
{-| ok -}
type alias GameMsgHandler m a msg = msg -> World m a msg
                                  -> (World m a msg, List (GameMessage m a msg))

{-| ok -}
initWorld : m  -> GameMsgHandler m a msg
          -> World m a msg
initWorld m msgHandler = World { meta = m
...
{-| Standard game object with physics attributes.
-}
type GameObject m a msg = GameObject
    { self : a
    , id : ObjectID
    , pos : Vector
    , vel : Vector
...
{-| ok -}
type GameMessage m a msg = NewObject (GameObject m a msg)
...
-}
type Mass = Mass Float | Fixed | Floating


{-| ok elm -}
type alias CollisionReaction m a msg = World m a msg -> GameObject m a msg -> GameObject m a msg
...
-}
type CollisionHandler m a msg = NoCollision
                        | CollisionHandler
...
-}
type ControlHandler m a msg = NoControl
                      | ControlHandler
                        (Time -> World m a msg -> GameObject m a msg
                        -> ( GameObject m a msg, List (GameMessage m a msg) ))

{-| Called every animation frame to draw the thing
-}
type DisplayHandler m a msg b = NoDisplay
...
{-| ok -}
handleGameMessage : GameMessage m a msg -> World m a msg
                  -> (World m a msg, List (GameMessage m a msg))
handleGameMessage message (World world) =
    case message of
        Debug str -> ( World { world | debug = str }, [] )
        NewObjectKeepID obj -> ( World { world | objects = obj :: world.objects }, [])
        NewObject (GameObject obj) ->
            let newID =  world.currentObjectID + 1
                objWithID = GameObject { obj | id = newID }
            in
                ( World { world | currentObjectID = world.currentObjectID + 1
                        , objects = objWithID :: world.objects }
                , [])
        Randoms n constructor ->
            let (rx, seed) = randoms n world.randomSeed []
            in
                world.msgHandler
                    (constructor rx)
...
randoms n seed xs =
    if (n == 0) then (xs, seed)
        ...
        let (x, nextSeed) = Random.step (Random.float 0 1) seed
        in
...
handleGameMessages messages world =
    case messages of
        ...
        (m::mx) ->
            let (world, msgs) = handleGameMessage m world
            in
                handleGameMessages (msgs ++ mx) world
...
updateWorld diff world =
    let (controlledWorld, controlMessages) = updatePhysics diff world
                                           |> resetPhysics
                                           |> updateControls diff
        (collidedWorld, collidedMessages) = updateCollisions diff controlledWorld
    in
...
updateControls diff (World world) =
    let (objects, messages) =
            List.foldr (\ (GameObject object) (objects, messages) ->
...
isZeroVector v = v.x == 0 && v.y == 0

clearDestroyed : World m a msg -> World m a msg
clearDestroyed (World world) =
    World
    { world | objects =
          (List.foldr (\ (GameObject obj) objects ->
                           if obj.destroy then objects
                           else (GameObject obj) :: objects)
               [] world.objects)}

resetPhysics : World m a msg -> World m a msg
resetPhysics (World world) =
    let updatedObjects =  List.map (\ (GameObject obj) ->
                                       GameObject
                                       { obj | forces = [] })
                          world.objects
    in
...
updatePhysics diff (World world) =
    let updatedObjects =  List.map (\ obj -> (updateObjectPhysics diff obj)) world.objects
    in
        World { world | objects = updatedObjects }

updateObjectPhysics : Time -> GameObject m a msg -> GameObject m a msg
updateObjectPhysics diff (GameObject object) =
    let sumForces = List.foldr vectorAdd (Vector 0 0) object.forces
        acc = (vectorDivScalar object.mass sumForces)
        vel = vectorAdd (vectorMultScalar diff (aveVec object.acc acc)) object.vel
        pos = vectorAdd (vectorMultScalar diff (aveVec object.vel vel)) object.pos
    in
    GameObject
    { object
        | acc = acc
...
aveVec v1 v2 = vectorAdd v1 v2
               |> vectorDivScalar 2


makeBounded : GameObject m a msg -> Bounded (GameObject m a msg)
makeBounded (GameObject obj) = { pos = obj.pos
                               , shape = obj.shape
        ...
        _ ->
            ...
            in
                case (List.map maybeUpdateBoundingBox collidables) of
                    [] -> ( World world, [] )
                    (x::xs) ->
                        let (updatedObjects, msgs) = checkCollisions diff
                                                     (World world) [] [] x xs
                        in
                            ( World { world | objects = updatedObjects }
                            , msgs )
...
checkCollisions diff world msgs checked current others =
    case others of
        ...
        _ ->
            let (updatedCurrent, updatedOthers, updatedMsgs) =
                    (List.foldr
                         (\ (GameObject oth) ((GameObject cur), uOthers, uMsgs) ->
                              let noCollision = (GameObject cur
                                                , (GameObject oth)::uOthers
                                                , uMsgs)
                              in
                                  case (cur.collisionHandler, oth.collisionHandler) of
                                      (NoCollision, _) -> noCollision
                                      (_, NoCollision) -> noCollision
                                      (CollisionHandler cHandler, CollisionHandler oHandler) ->

                                          if (cHandler.canCollideWith oth.self
                                             || oHandler.canCollideWith cur.self) then
            ...
            in
                case updatedOthers of
                    [] -> (updatedCurrent :: checked, updatedMsgs) --should never happen
                    (oth::oths) ->
                        checkCollisions
                            diff world (msgs ++ updatedMsgs)
...
areColliding (GameObject o1) (GameObject o2) =
    ...
    case (o1.shape, o2.shape) of
        (Point, Point) -> False
        (Point, Circle r) -> (o1.pos.x - o2.pos.x)^2 + (o1.pos.y - o2.pos.x)^2 < r ^2
        (Point, Polygon points) -> o1.pos.x > o2.boundingBox.xSpan.low
                                   && o1.pos.x < o2.boundingBox.xSpan.high
                                   && o1.pos.y > o2.boundingBox.ySpan.low
                                   && o1.pos.y < o2.boundingBox.ySpan.high
        (Circle _, Point) -> areColliding (GameObject o2) (GameObject o1)
        (Circle r1, Circle r2) -> (o1.pos.x - o2.pos.x)^2 + (o1.pos.y - o2.pos.x)^2
                                  < (r1 + r2)^2
        (Circle r1, Polygon points) -> False

        (Polygon points1, Polygon points2) ->
            boundingBoxCollision o1.boundingBox o2.boundingBox

        (Polygon _, Point) -> areColliding (GameObject o2) (GameObject o1)
        (Polygon _, Circle _) -> areColliding (GameObject o2) (GameObject o1)
