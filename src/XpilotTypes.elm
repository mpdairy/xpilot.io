...
type XpilotObject = Bullet { shooter : ObjectID
                           , shotLife : Time }
                  | Ship { dir : Float
--                         , id : Int
                         , shape : List Vector
                         ...
                         , brain : ShipBrain}
                  | Wall {}
                  | Ember { strength : Float
                          , age: Time
                          , owner: ObjectID }

type XpilotMsg = Explosion Vector Vector (List Float)
...
type ControlAction = Thrust | TurnRight | TurnLeft | Shoot

type alias ControlMapping = Dict KeyCode ControlAction

type ShipControls = ShipControls { turn : Float
                                 , thrust : Float
                                 , shoot : Bool }

type alias ShipBrain = World XpilotMeta XpilotObject XpilotMsg
