...
emptyStats = { kills = 0
             ...
             , lifeScore = 0 }
--
type Player = Player { id : ObjectID
                     , name : String
                     , dead : Bool
                     , timeDead : Time
                     , stats : Stats
...
--
type DisplayMsg = DisplayMsg { timeDisplayed : Time
                             , message : String
                             , color : String }
...
init =
    let
        meta = { keyboardControls =
                     (Dict.fromList [ (16, Thrust)
                                    , (65, TurnRight)
                                    , (83, TurnLeft)
                                    , (13, Shoot)
                                    , (32, Shoot)
        ...
        (World world) = initWorld meta xpilotMsgHandler
        --(GameObject player) = (newShip meta 7 0 (Vector 400 400) keyboardBrain)
        screenSize = (Vector 1200 700)
        screenZoom = 1.3
        playSize = vectorDivScalar screenZoom screenSize
        gameState =
...
calcOffset playSize playerPos = (vectorAdd (vectorMultScalar 0.5 playSize)
                                     (vectorMultScalar -1 playerPos))
--
updateHud : XpilotMeta -> GameState -> Maybe Player
          -> Maybe (GameObject XpilotMeta XpilotObject XpilotMsg) -> Hud -> Hud
updateHud meta gs maybePlayer maybePlayerObj hud =
    case maybePlayer of
        Nothing -> hud
        Just (Player p) ->
            let energy = case maybePlayerObj of
                             Nothing -> hud.energy
                             Just (GameObject obj) ->
                                 case obj.self of
                                     (Ship s) ->
                                     ...
                                     _ -> hud.energy
                timeToRespawn = if p.dead then (meta.playerDeadTime - p.timeDead) / Time.second
                                else 0
            in
                { hud | energy = energy
...
hudMessage hud color s = { hud | messages =
                         hud.messages ++ 
                         [(DisplayMsg { timeDisplayed = 0
                                      , message = s
                                      , color = color })] }
--
gameStateMessageHandler : ObjectID -> GameState -> XpilotMeta
        ...
        GameEngine.Game xpmsg ->
            case xpmsg of
                Killed killedID killerID killerObj ->
                    case killerObj of
                        Ship ship ->
                            ({ gs | players = Dict.update killedID
                                   (\ p -> case p of
                                               Nothing -> Nothing
                                               Just (Player pl) ->
                                                   Just (Player
                                                             (let stats = pl.stats
                                                                  ustats = { stats | deaths = stats.deaths + 1
...
--
updateGameState : Time -> ObjectID -> GameState -> XpilotMeta
                -> List (GameMessage XpilotMeta XpilotObject XpilotMsg)
                -> (GameState, List (GameMessage XpilotMeta XpilotObject XpilotMsg))
updateGameState diff selfID gs meta msgs =
    let (ugs, umsgs) = List.foldr
                       (\msg (fgs, fmsgs)->
                            let (rgs, rmsgs) = gameStateMessageHandler selfID fgs meta msg
                            in
                                (rgs, rmsgs ++ fmsgs))
                       (gs, [])
                       msgs
        (tplayers, tmsgs) = tickPlayers diff meta ugs.players
        deadplayers = Dict.map (\ _ (Player p) -> p.dead) ugs.players
        hud = ugs.hud
        uhud = { hud | messages = tickHudMessages diff hud.messages }
    in
        ({ugs | players = tplayers
         , hud = uhud}
        , tmsgs ++ umsgs)
--
tickHudMessages : Time -> List DisplayMsg -> List DisplayMsg
tickHudMessages diff msgs =
    List.foldr (\ (DisplayMsg m) mx ->
...
--
tickPlayers : Time -> XpilotMeta -> Dict ObjectID Player
            -> (Dict ObjectID Player, List (GameMessage XpilotMeta XpilotObject XpilotMsg))
tickPlayers diff meta players =
    Dict.foldr (\ id (Player p) (ps, msgs) ->
                    if p.dead then
                        let newTimeDead = p.timeDead + diff
                            respawn = newTimeDead >= meta.playerDeadTime
                            stats = p.stats
                        in
                            (Dict.insert id
                                 (Player {p | timeDead = newTimeDead
                                         , dead = not respawn
                                         , stats = if respawn then
                                                       {stats | lifeKills = 0}
                                                   else
                                                       stats
                                         })
...
initialSizeCmd =
    Task.perform (\_ -> Error "OK") (Window True) Window.size
...
update msg model =
  case msg of
      ...
      Window init {width, height} ->
          let screenSize = Vector (toFloat width) (toFloat height)
              zoom = if init then screenSize.y / 630 else model.screenZoom
          in
      ...
      KeyDown k ->
          ( { model | gameWorld = handleKeyDown k model.gameWorld
            , splash = False
            , donate = False }
      ...
      Tick tdiff ->
          if model.paused then
              ...
              let
                  diff = Basics.min tdiff (50 * Time.millisecond)
                  (World world, gameMessages) = updateWorld diff model.gameWorld
                  (gs, gsMsgs) = updateGameState diff model.selfID model.gameState world.meta gameMessages
                  (World uworld) = handleGameMessages (gsMsgs ++ gameMessages) (World world)
                  maybePlayerObj = getPlayer model.selfID world.objects
                  maybePlayer = Dict.get model.selfID gs.players
                  offset = case maybePlayerObj of
                               Nothing -> model.offset
                               Just (GameObject p) -> calcOffset model.playSize p.pos
                  hud = updateHud uworld.meta gs maybePlayer maybePlayerObj gs.hud
                  hgs = { gs | hud = hud }
                  deadFetchCmd = case (model.statsID, maybePlayer) of
                                     (Nothing, _) -> Cmd.none
                                     (_, Nothing) -> Cmd.none
                                     (Just statsID, Just (Player p)) ->
                                         if p.dead && p.timeDead <= diff then
                                             ...
                                             Cmd.none
                  maybeDonate = Maybe.withDefault Cmd.none <|
                                Maybe.map (\ (Player p) ->
                                               if p.dead && p.timeDead <= diff then
                                                   Random.generate
                                                       Donate
                                    ...
                                    maybePlayer
              in
                  ( { model | gameWorld = (World uworld)
                    , gameState = hgs
                    , offset = offset}
                  , Cmd.batch [deadFetchCmd
                              --, maybeDonate
                              ] )

      Game gameMessages ->
          let world = handleGameMessages gameMessages model.gameWorld
          in
              ( { model | gameWorld = world }
              , Cmd.none )

      StatsNewGame sid -> { model | statsID = Just sid } ! []

      StatsDeath _ -> model ! []

      Visible v -> case v of
                       PV.Visible -> { model | paused = False } ! []
                       PV.Hidden -> { model | paused = True } ! []

      Donate (a, b) -> if a == 0 then
                           { model
                               | donate = True
                               , donateMsg = Maybe.withDefault "" <|
...
fetchNewGame =
  Http.getString "/newgame"
    ...
    |> Task.perform Error StatsNewGame
--
...
view model =
   -- drawScene model
    div [ Attr.style [ ( "height", toString (model.screenSize.y) )
                     , ( "width", toString (model.screenSize.x) )
                     , ( "overflow", "hidden") ] ]
        <| List.concat [ [ drawScene model ]
...
drawScene model =
    let (World world) = model.gameWorld
    in
        (Svg.svg
                      [ width (toString (model.screenSize.x))
...
drawObject selfID offset (GameObject object) =
    if object.destroy == False then
        case object.self of
            Ship _ -> drawShip selfID offset (GameObject object)
            Bullet _ -> drawBullet offset (GameObject object)
            Wall _ -> drawWall offset (GameObject object)
            Ember _ -> drawEmber offset (GameObject object)
    else []

drawText : Vector -> String -> Html Msg
drawText pos str =  Svg.text' [ x (toString pos.x)
                                 , y (toString pos.y)
                                 , fill "white"]
                           [ Svg.text str ]
...
drawBullet offset (GameObject bullet) =
    [Svg.rect [ x (toString (bullet.pos.x + offset.x))
...
drawEmber offset (GameObject ember) =
    [Svg.rect [ x (toString (ember.pos.x + offset.x - 0.8))
...
drawShape offset styles shape = case shape of
                      Polygon pointlist ->
                          drawPolygon offset styles pointlist
                      _ -> drawPoint offset (Vector 0 0)


drawPoint : Vector -> Vector -> Html Msg
drawPoint offset point = 
    Svg.rect [ x (toString (point.x + offset.x))
...
drawPolygon offset styles pointlist =
    let pointstring = (List.foldl (\ point s ->
                                      s ++
...
drawHud screensize hud =
    let middle = vectorMultScalar 0.5 screensize in
    List.concat
        [ [Svg.rect [ x (toString (middle.x - hudWidth / 2))
                    , y (toString (middle.y - hudHeight / 2))
                    , width (toString hudWidth), height (toString hudHeight)
                    , stroke "green"
                    , fill "none"
...
drawRadar model =
    let (World world) = model.gameWorld
        rx = model.playSize.x - radarWidth - 1
        ry = model.playSize.y - radarHeight - 1
    in
        [Svg.svg  [ x (toString rx)
                  , y (toString ry)
                  , width (toString radarWidth)
                  , height (toString radarHeight)
                  , viewBox ( (toString (1 * model.playSize.x / 2 - 700)) ++ " "
                                 ++ (toString (1 * model.playSize.y / 2 - 700)) ++ " "
                                 ++ (toString 1400) ++ " "
                                 ++ (toString 1400))
--                  , viewBox "0 0 1200 1200"
                 ]
             ([Svg.rect [ x (toString (1 * model.playSize.x / 2 - 700))
                        ...
                        , strokeWidth "13"] []]
                  ++
                  (List.foldr (\ x xs -> case drawRadarObject
                                   (Vector rx ry)
...
drawRadarObject : Vector -> Vector -> Float -> Vector
                -> GameObject XpilotMeta XpilotObject XpilotMsg -> Maybe (Html Msg)
drawRadarObject rpos rsize rzoom offset (GameObject obj) =
    case obj.self of
        (Ship _) -> Just ( Svg.rect [ x (toString (obj.pos.x + offset.x))
                                    , y (toString (obj.pos.y + offset.y))
                                    , width "15", height "15"
                                    , fill "white"
                                    ] [] )
        (Wall _) -> Just ( drawShape (vectorAdd offset obj.pos)
                                  [fill "#2222dd"]
                                  obj.shape )
        _ -> Nothing
--
...
drawScores model =
    let gs = model.gameState
        players = Dict.foldr (\ _ p ps -> p :: ps)
                  []
                  gs.players
        splayers = List.sortBy (\ (Player p) -> (p.stats.kills - p.stats.deaths) * -1 ) players
    in
        Svg.svg  [ x <| toString 10
                 , y <| toString 10
                 , width <| toString scoresWidth
                 , height <| toString scoresHeight
                 , viewBox <| "0 0 " ++ (toString scoresWidth)
...
printPlayerScore offset i (Player p) =
    Svg.svg [ x <| toString offset.x
        ...
        , Svg.text' [ fill "#eee"
                    ...
                    , x <| toString 100
                    , y <| toString <| 15 ]
              [ Svg.text <| (String.left 3
                    <| toString ((toFloat p.stats.kills)
...
welcomeSplash screensize =
    let middle = vectorMultScalar 0.5 screensize in
    Svg.svg [ x <| toString <| middle.x - splashWidth / 2
...
--
wants : List String
wants = [ "Want to fight against human players instead of bots?"
...
drawDonateSplash screensize msg =
    ...
    , div [ Attr.style [ ( "color", "yellow")
          ...
          [ a [ Attr.href "https://www.kickstarter.com/projects/196267783/xpilotio"
